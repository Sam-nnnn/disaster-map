...
export function TopBar() {
  ...
  return (
    <div className="absolute top-5 left-0 right-0 z-[1200] pointer-events-none px-5 flex items-center justify-between">
      {/* Search Input */}
      <div className="relative flex-1 max-w-[460px] shadow-[0_2px_15px_rgba(0,0,0,0.12)] rounded-full bg-white/70 backdrop-blur border border-slate-100/50 dark:bg-slate-900/70 border border-slate-100/50 dark:border-slate-800 group pointer-events-auto transition-all">
        <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none">
          <Search
            className="h-[18px] w-[18px] text-slate-400 group-focus-within:text-blue-400 transition-colors"
            strokeWidth={2.5}
          />
        </div>
        <input
